...
repo.dropTemp("fixedFood")
repo.createTemp("fixedFood")
title = ''
for idx, u in enumerate(r2):
	if idx == 0:
		title = r2[idx]['FIELD6']
	elif r2[idx]['FIELD6']:
		repo['jlam17_mckay678.fixedFood'].insert({'Location': r2[idx]['FIELD6']})

for idx, u in enumerate(r):
	if idx == 0:
		title = r[idx]['FIELD13']
	elif r[idx]['FIELD13']:
		repo['jlam17_mckay678.fixedFood'].insert({'Location': r[idx]['FIELD13']})

for idx, u in enumerate(r3):
	if idx == 0:
		title = r3[idx]['FIELD4']
	elif r[idx]['FIELD4']:
		repo['jlam17_mckay678.fixedFood'].insert({'Location': r3[idx]['FIELD4']})
...
for idx, u in enumerate(r5):
	if idx == 0:
		...
		repo['jlam17_mckay678.fixedFood'].insert({'Location': (r5[idx]['FIELD10'], r5[idx]['FIELD11'])})
...
provEx = open('http://upload.datamechanics.org/?prefix=jlam17_mckay678/provFoodLocation.json', 'w')
provEx.write(json.dumps(json.loads(doc.serialize()), indent=4))
prov2 = open('http://upload.datamechanics.org/?prefix=jlam17_mckay678/planFoodLocation.json', 'w')
