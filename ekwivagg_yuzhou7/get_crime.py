...
url = 'https://data.cityofboston.gov/resource/7cdf-6fgx.json?incident_type_description=larceny+from+motor+vehicle&$select=location&$limit=50000'
...
for larceny in larcenies:
    ...
    larceny_loc.append((float(latitude), float(longitude)))

larceny_stop = []
for larceny in larceny_loc:
    min_lat = round(larceny[1], 1)
    min_long = round(larceny[2], 1)
    for stop in stop_loc:
        if round(stop[1], 1) == min_lat and round(stop[2], 1) == min_long:
...
for pair in closest_stop:
    closest_t_stop[pair[0]] = {'tstop': pair[1], 'distance':pair[2]}
